...
2. Spawn one `fable-flow:implementer` per track, **all in a single message** so they run concurrently — each already gets its own isolated worktree. Each prompt carries, inline: its full track manifest, the entire Contracts section, the conventions digest (`.fable-flow/explore-conventions.md`), `BASE_SHA`, and branch slug `fable-flow/<SLUG>-t<n>`. Remind it: reset to `BASE_SHA` first, touch only owned files, commit before reporting. Save reports to `.fable-flow/track-<n>-report.md`.
