...
7. **Boundaries, stated.** Dirty trees are refused rather than stashed; `/fable-flow:review` without `--fix` reports and stops; PRs require `--pr`. Fable takes initiative — the prompts define where it must not.
   - **Interactive up front, then hands-off.** `/fable-flow:ship` front-loads the human's leverage: any new-project brainstorm and a single **plan sign-off** happen before implementation, and once the user approves the plan the run goes hands-off to completion (implement → merge → review → ship) with no further check-ins — the plan is the cheap place to change course, so that's where the gate is. `--auto` drops the plan gate for a fully unattended run.
...
**Clarifying intent before planning — two modes.** For a new or greenfield project, the Plan phase (in both `/fable-flow:plan` and `/fable-flow:ship`) settles the open design and direction decisions with the user *before* the architect plans. It surfaces **every** open decision — not a truncated few — as card-based multiple-choice questions (one decision per card, a recommended option first, and an option preview — an ASCII mockup, snippet, or layout sketch — where a visual side-by-side helps), across as many rounds as it takes until nothing material is unresolved. Facts are looked up, not asked; only genuine decisions go to the user; answers fold into `.fable-flow/task.md`. This is the default light clarification and it's skipped for a well-specified change to existing code. The heavier [skills/grilling/SKILL.md](skills/grilling/SKILL.md) — a relentless one-question-at-a-time interrogation of the whole design tree (adapted from Matt Pocock) — is **opt-in only**: its description gates it to when the user explicitly asks to be "grilled", so it never fires on ordinary planning.
