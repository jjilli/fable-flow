...
| Skills | `skills/fable-prompting/` — the distilled Fable 5 prompting reference · `skills/build-patterns/` — battle-tested engineering patterns and seam traps from long multi-round builds (the recurring "green per track, broken at the seam" failure, reusable patterns, verification discipline beyond a green suite) · `skills/frontend-aesthetics/` — how to build UI that looks designed instead of generic, plus headless-screenshot visual verification · `skills/grilling/` — opt-in relentless one-question-at-a-time interrogation of a plan (invoked only when the user asks to be "grilled"; adapted from Matt Pocock) |
