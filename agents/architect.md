...
- **For user-facing work, commit to a design direction.** When the task builds or restyles UI, state the aesthetic (type, palette, one motion moment) in the plan and make a track own the shared design system before the individual pages — restyling shared classes lifts every page at once. Require a headless-browser screenshot against seeded data in Integration verification. The `frontend-aesthetics` skill has the specifics.
